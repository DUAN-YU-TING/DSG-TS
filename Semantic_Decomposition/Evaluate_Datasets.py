...
domain_index = 0
length = 24
df = pd.read_csv(os.path.join('./TSFragment-600K', f'embedding_cleaned_{domain[domain_index]}_{length}.csv'), index_col=False)
...
final_df.to_csv(os.path.join('./Data/Refined_Data', f'{domain[domain_index]}_{length}_refined.csv'),
                index=False)
